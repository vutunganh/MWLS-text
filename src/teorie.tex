...
K-d strom je binární vyhledávací strom, jehož klíči v jednotlivých vrcholech jsou vektory z $\reals{}^k$. Levého potomka vrcholu $P$ značíme $L(P)$, pravého $R(P)$. Diskriminátor vrcholu $P$ je číslo z množiny $\{0, \ldots, k-1\}$ a značíme $D(P)$. Vrchol $P$ dokáže udržovat vektory v rozsahu $B_L(P) \in \reals^k$ až $B_U(P) \in \reals^k$.
