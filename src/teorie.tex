...
\end{equation}
kde $b_j$ jsou složky báze vektorového prostoru polynomů $\prod^d_m$ a $\mx{A}$ je matice neznámých koeficientů. V obecném případě je dimenze vektorového prostoru $\prod^d_m$ dána vztahem $k = \frac{(d+m)!}{d!m!}$.\cite{mwls-approximation-power, classification-of-meshfree}
