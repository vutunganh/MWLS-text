...
\begin{equation}
  ...
  \sum_{i=1}^k\sum_{u=1}^m(\sum_{j=1}^l c_j(f_j(\vec{x_i}))_u - (\vec{y_i})_u).
\end{equation}

Minimalizační úlohu \ref{def:ls-min} lze řešit pomocí nutné podmínky pro existenci lokálního extrému. Položme parciální derivace funkce $J(f)$ rovny nule pro neznáme $c_1,\ldots,c_m$. Pro obecné $p, q \in \integers$ platí
\begin{multline}
...
\end{multline}
Nyní položme tyto derivace pro všechna $\mx{A}_{1,1}, \ldots, \mx{A}_{o, k}$ rovny 0. Tímto vznikne soustava rovnic
\begin{multline}
\end{multline}
TODO: dukaz
