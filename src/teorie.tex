...
Složitost algoritmu \ref{algo:kd-tree-range-search} se obtížně určuje a proto tuto analýzu přeskočíme. Lee a Wong v \cite{kd-tree-worst} ukázali, že v nejhorším případě je tato složitost $O(kN^{1-1/k})$, kde $N$ je počet záznamů ve stromě a $k$ je dimenze, ve níž se pracuje. Důsledkem tohoto výsledku je, že s rostoucí dimenzí se asymptotická složitost přibližuje k naivnímu vyhledávání přes celou množinu dat $S$.
